...
public class Edit extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        String res_hint = "";
        try {
            String typeName = request.getParameter("edit_type_name");
            int cover = Integer.parseInt(request.getParameter("edit_cover"));
            String typeComment = request.getParameter("edit_type_comment");
            int target = Integer.parseInt(request.getParameter("edit_type_target"));
            Database database = Database.connect(Config.readConfFromFile(request));
            switch (ItemData.editItemType(database, target, typeName, cover, typeComment)) {
                case 0:
