package top.chorg.iCommerce.response.admin.order;
...
public class Send extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        String res_hint = "";
        try {
            String expressNum = request.getParameter("express");
            int target = Integer.parseInt(request.getParameter("sendTarget"));
            Database database = Database.connect(Config.readConfFromFile(request));
            if (OrderData.setPhase(database, target, 2) + OrderData.setExpress(database, target, expressNum) == 0) {
        ...
        Request.becomeInfoPage(request, response, res_type, "订单管理", res_str, res_hint);
    }
}
